...
const CategoryPopup = ({ itemName, isCategory }: { itemName: string, isCategory: boolean }) => {
  ...
  return (
    <HoverCard openDelay={100} closeDelay={300}>
      ...
      <HoverCardContent
        className="w-48 bg-[#0F0828] overflow-hidden rounded-2xl border-none p-0 shadow-lg shadow-black relative before:content-[''] before:absolute before:-top-2 before:left-1/2 before:-translate-x-1/2 before:w-0 before:h-0 before:border-l-8 before:border-r-8 before:border-b-8 before:border-l-transparent before:border-r-transparent before:border-b-[#16151A]"
        align="center"
        sideOffset={8}
      >
        <div>
          {categories.map((category) => (
            <Link
              key={category.name}
              href={category.href}
              className="block px-4 py-3 text-white text-sm hover:bg-[#080414] transition-colors"
            >
              {category.name}
            </Link>
          ))}
