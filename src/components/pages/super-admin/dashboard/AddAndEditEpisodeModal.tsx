...
export function AddAndEditEpisodeModal({ open, onOpenChange, mode, episodeData, onSave }: AddAndEditEpisodeModalProps) {
  const {control,
    handleSubmit,
    reset,
    formState: { errors },} = useForm<EpisodeFormData>({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] max-w-[400px] h-auto max-h-[80vh] p-0 m-4 flex flex-col [&>button]:rounded-full [&>button]:border-2 [&>button]:border-gray-800 [&>button]:p-[2px]">
        <DialogHeader className="px-4 sm:px-6 py-4  flex-shrink-0">
          <DialogTitle className="text-lg text-[#242424] font-medium">
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <FormInputField control={control} name="episodeNumber" 
              label="Episode Number" placeholder="10"
              ...
              <div className="pt-4">
                <Button type="submit" className="w-full bg-black hover:bg-gray-800 text-white rounded-full py-3">
                  Submit
                </Button>
              </div>
            </form>
